...
#data paths
DATA_PATH = "./data/USA/usa_matrix_50k.csv"
DATA_FORMAT = "csv"
DATES_PATH  = "./data/USA/usa_int_dates_50k.csv"
DATES_FORMAT = "int"
...
WINDOW_SIZE= 100 #number of sequences in the sliding window
...
def main():
    ...
    print(f"{WINDOW_SIZE=},{sequence_length=}, flatsize={WINDOW_SIZE*sequence_length}")
...
def train(model, train_loader, test_loader):
    ...
    for epoch in tqdm(range(EPOCHS)):
        model.train()
        for i, (slides, targets) in tqdm(enumerate(train_loader), leave=False, total=len(train_loader)):
...
def createDataLoaders(train_data, test_data, val_data):
    ...
    test_loader = torch.utils.data.DataLoader(test_ds, batch_size=BATCH_SIZE, shuffle=True, num_workers=WORKERS)
    val_loader = torch.utils.data.DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=True, num_workers=WORKERS)
